# Test unlearnable examples by training classifiers on them 
...
# Classifier Options
parser.add_argument('--turn_on_robust', action='store_true', default=False, help='Whether randomize classifier parameters during training')
...
def train(starting_epoch, model, optimizer, scheduler, criterion, trainer, evaluator, ENV, data_loader):
    ...
    for epoch in range(starting_epoch, config.epochs):
        ...
        # Train
        if args.turn_on_robust and switch:
            ENV['global_step'] = trainer.train(epoch, model, optimizer, args.robust_noise, 
                                               args.robust_noise_step, args.avgtimes, 
                                               pue=args.turn_on_robust)
        else:
            ENV['global_step'] = trainer.train(epoch, model, optimizer)
        val_acc = trainer.acc_meters.avg*100
        ENV['train_history'].append(val_acc)
        if val_acc > 50:
                switch = 1
        scheduler.step()
