...
transform_options['ImageNette'] = transform_options['ImageNet']
transform_options['PoisonImageNette'] = transform_options['ImageNet']
transform_options['ImageNet'] = transform_options['ImageNet']
transform_options['PoisonImageNet'] = transform_options['ImageNet']
...
class DatasetGenerator():
    def __init__(self, train_batch_size=128, eval_batch_size=256, num_of_workers=4,
        ...
        elif train_data_type == 'ImageNette':
            num_of_classes = 10
            train_dataset = ImageNette(root=train_data_path, split='train',
                                       transform=train_transform)
        elif train_data_type == 'PoisonImageNette':
            num_of_classes = 10
            train_dataset = PoisonImageNette(root=train_data_path, split='train', seed=seed,
                                             transform=train_transform, poison_rate=poison_rate,
                                             perturb_tensor_filepath=perturb_tensor_filepath)
        ...
        if test_data_type == 'CIFAR10':
            ...
            test_dataset = datasets.ImageNet(root=test_data_path, split='val',
                                             transform=test_transform)
        elif test_data_type == 'ImageNette':
            test_dataset = ImageNette(root=test_data_path, split='val',
                                      transform=test_transform)
        elif test_data_type == 'PoisonImageNette':
            test_dataset = PoisonImageNette(root=test_data_path, split='val', seed=0,
    ...
    def getDataLoader(self, train_shuffle=True, train_drop_last=True, test_drop_last=False):
        data_loaders = {}
        train_sampler = RandomSampler(self.datasets['train_dataset'])
        ...
        data_loaders['test_dataset'] = DataLoader(dataset=self.datasets['test_dataset'],
                                                  ...
                                                  drop_last=test_drop_last, num_workers=self.num_of_workers)

        return data_loaders

    def getRecoveryDataLoader(self, train_shuffle=True, train_drop_last=True, test_drop_last=False):
        data_loaders = {}
        train_sampler = RandomSampler(self.datasets['test_dataset'])
        data_loaders['recovery_dataset'] = DataLoader(dataset=self.datasets['test_dataset'],
                                                   batch_size=self.train_batch_size,
                                                   #shuffle=train_shuffle, 
                                                   pin_memory=True,
                                                   sampler=train_sampler,
    ...
    def _split_validation_set(self, train_portion, train_shuffle=True, train_drop_last=True, test_drop_last=False):
        np.random.seed(self.seed)
        train_subset = copy.deepcopy(self.datasets['train_dataset'])
        valid_subset = copy.deepcopy(self.datasets['train_dataset'])

        if self.train_data_type == 'ImageNet' or self.train_data_type == 'ImageNetMini' or self.train_data_type == 'TinyImageNet' or self.train_data_type == 'PoisonImageNetMini' or self.train_data_type == 'ImageNette' or self.train_data_type == 'PoisonImageNette':
            ...
            print(f"Check! Data lenghth: {len(self.datasets['train_dataset'].data)}, targets length: {len(self.datasets['train_dataset'].targets)}")
        ...
        data_loaders = {}
        train_sampler = RandomSampler(self.datasets['train_dataset'])
        train_subset_sampler = RandomSampler(self.datasets['train_subset'])
        data_loaders['train_dataset'] = DataLoader(dataset=self.datasets['train_dataset'],
                                                   batch_size=self.train_batch_size,
                                                   #shuffle=train_shuffle, 
                                                   pin_memory=True,
    ...
    def _split_train_proxy_validation_set(self, train_portion, proxy_rel_portion, train_shuffle=True, train_drop_last=True, test_drop_last=False, random_seed = None):
        np.random.seed(self.seed)
        train_subset = copy.deepcopy(self.datasets['train_dataset'])
...
class PoisonCIFAR10(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        print('perturbation tendor shape:', self.perturb_tensor.shape)
        ...
        print('current target dim', target_dim)
...
class ImageNetMini(datasets.ImageFolder):
    def __init__(self, root, split, transform, **kwargs):
        self.split = split
        if split == "train":
            root = root + '/imagenet-100/train'
        elif split == "val":
            root = root + '/imagenet-100/val' 
        super(ImageNetMini, self).__init__(root=root, transform=transform, **kwargs)
        print(f'ImageNet-100 Loaded. Number of samples in the {split} set is {len(self)}.')
...
class ImageNette(datasets.ImageFolder):
    def __init__(self, root, split, transform, **kwargs):
        self.split = split
        if split == "train":
            root = root + '/imagenette/train'
        elif split == "val":
            root = root + '/imagenette/val' 
        super(ImageNette, self).__init__(root=root, transform=transform, **kwargs)
        print(f'ImageNette Loaded. Number of samples in the {split} set is {len(self)}.')


class PoisonImageNette(ImageNette):
    def __init__(self, root, split, poison_rate=1.0, seed=0,
                 perturb_tensor_filepath=None, **kwargs):
        super(PoisonImageNette, self).__init__(root=root, split=split, **kwargs)
        np.random.seed(seed)
        self.poison_rate = poison_rate
        self.perturb_tensor = torch.load(perturb_tensor_filepath)
        self.perturb_tensor = self.perturb_tensor.mul(255).clamp_(0, 255).permute(0, 2, 3, 1).to('cpu').numpy()
